...
# Check which directories are in the Languages
# folder
directory = './Languages'
available_languages = [f[1] for f in os.walk(directory)][0]
disp_languages = ', '.join(available_languages)

expInfo['languages '+ disp_languages] = ''

## Create new dialogue box
#myDlg = gui.Dlg(title="expInfo")
#myDlg.addText('Experiment info')
#myDlg.addField('participant ')
## Display available languages in dialogue box
#myDlg.addField('language \n(available: ' + disp_languages+ ')')
#myDlg.addField('parallel port')
...
expInfo = {'participant': '', 'language': 'English', 'parallel port': '03F8'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Stimulus\\Documents\\Psychopy\\restingStatePresentation-main\\Ruhe_EEG.py',
...
for thisResting_trial in Resting_trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if lpt_port.status == NOT_STARTED and t >= 0.0-frameTolerance:
            ...
            win.callOnFlip(lpt_port.setData, int(Trigger))
        if lpt_port.status == STARTED:
            ...
            if tThisFlipGlobal > lpt_port.tStartRefresh + 0.015-frameTolerance:
                ...
                win.timeOnFlip(lpt_port, 'tStopRefresh')  # time at next scr refresh
    ...
    if lpt_port.status == STARTED:
        win.callOnFlip(lpt_port.setData, int(0))
    Resting_trials.addData('lpt_port.started', lpt_port.tStart)
    Resting_trials.addData('lpt_port.stopped', lpt_port.tStop)
